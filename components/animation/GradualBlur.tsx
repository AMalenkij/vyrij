...
type ResponsiveKeys = "height" | "width";
...
const GradualBlur: React.FC<GradualBlurProps> = (props) => {
  ...
  const blurDivs = useMemo(() => {
    ...
    const currentStrength =
      isHovered && config.hoverIntensity
        ? (config.strength ?? 1) * config.hoverIntensity
        : (config.strength ?? 1);
    ...
    for (let i = 1; i <= (config.divCount ?? 1); i++) {
      ...
      if (config.exponential) {
        ...
        blurValue =
          0.0625 * (progress * (config.divCount ?? 1) + 1) * currentStrength;
      }
      ...
      const direction = getGradientDirection(config.position ?? "top");
  ...
  const containerStyle: CSSProperties = useMemo(() => {
    ...
    if (isVertical) {
      ...
      baseStyle[config.position ?? "top"] = 0;
      baseStyle.left = 0;
      baseStyle.right = 0;
  ...
  useEffect(() => {
    if (
      ...
      const t = setTimeout(
        () => config.onAnimationComplete?.(),
        parseFloat(config.duration ?? "0") * 1000,
      );
